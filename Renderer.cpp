...
double tx1, ty1, tz1, tx2, ty2, tz2, tx3, ty3, tz3, dx12, dx13, dx23, dx31, dx32, dy12, dy13, dy23, dy31, dy12_dx01, dy23_dx02,
        dy31_dx03, dy23_dx03, lambda1, lambda2, lambda3, depth, minX, minY, maxX, maxY;
map<int, double>::iterator i;
double lightX = 100, lightY = 100, lightZ = 100, tCenterX, tCenterY, tCenterZ, distanceFromLight, intensity;
...
double Renderer::render(unsigned char *colorBuffer) {
    ...
    for (const auto &triangle : scene->triangles) {
        ...
        if (Vector3::dot(tx1, ty1, tz1, triangle.normalVector) >= 0) {
            ...
            maxX = std::min(maxX, imageWidth - 1.0);
            ...
            for (auto x = static_cast<int>(minX); x < maxX; x++) {
                ...
                maxY = std::min(maxY, imageHeight - 1.0);

                for (auto y = static_cast<int>(minY); y < maxY; y++) {
...
void Renderer::setColor(const Color &color, int index, unsigned char *colorBuffer, double intensity) const {
